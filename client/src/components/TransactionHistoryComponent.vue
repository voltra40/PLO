		...
		<div class="columns is-centered">
			...
			<div class="column is-narrow">
				...
				<div class="tags are-large has-addons">
					...
					<span v-if="change < 0" class="tag is-danger">
						{{ change.slice(0, 1) + "$" + change.slice(1) }}
					</span>
					<span v-else class="tag is-success"> {{ "$" + change }}</span>
				</div>
			</div>
		</div>
	</div>
...
export default {
	...
	async mounted() {
		...
		for (let i = 0; i < this.data.length; i++) {
			...
			tempAverage +=
				parseFloat(this.data[i][2]).toFixed(2) *
				parseFloat(this.data[i][3]).toFixed(2);
			totalEthereum += parseFloat(this.data[i][1]);
		}

		this.averagePrice = (tempAverage / total).toLocaleString("en-US", {
		...
		this.totalETH = totalEthereum.toLocaleString("en-US", {
			minimumFractionDigits: 2,
			maximumFractionDigits: 2,
		});
		...
		this.currentPrice = parseFloat(
			currentPrice.data["1027"].quote.USD.price.toFixed(2)
		).toLocaleString("en-US");

		this.change = parseFloat(
			currentPrice.data["1027"].quote.USD.price.toFixed(2) - total
		).toLocaleString("en-US");
	},
};
</script>
